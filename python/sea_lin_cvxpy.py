...
for i in range(len(gamma_1)):
  constraints.append(gamma_1[i]*alpha+gamma_2[i]<=tau_m_max)
  constraints.append(-gamma_1[i]*alpha-gamma_2[i]<=tau_m_max)
  constraints.append(gamma_1[i]*alpha+gamma_2[i]<=K_t*V_m/R_m-K_m*K_m*r*(vel_data[i]-alpha*dtrq_data[i]))
  constraints.append(-gamma_1[i]*alpha-gamma_2[i]<=K_t*V_m/R_m-K_m*K_m*r*(vel_data[i]-alpha*dtrq_data[i]))
  constraints.append(gamma_1[i]*alpha+gamma_2[i]<=K_t*V_m/R_m+K_m*K_m*r*(vel_data[i]-alpha*dtrq_data[i]))
  constraints.append(-gamma_1[i]*alpha-gamma_2[i]<=K_t*V_m/R_m+K_m*K_m*r*(vel_data[i]-alpha*dtrq_data[i]))

n = len(trq_data)
constraints.append(cp.norm(I_m*r*(acc_data-alpha*ddtrq_data)+b_m*r*(vel_data-alpha*dtrq_data))/math.sqrt(n) <= tau_m_RMS)  # +mu_t*cp.sign(r*(vel_data-alpha*dtrq_data))

# Form objective.
obj = cp.Minimize(a_num*alpha**2 + b_num*alpha + c_num)
...
if prob.status == 'infeasible':
  ...
  tau_m_opt = I_m*r*(acc_data - alpha.value * ddtrq_data) + b_m*r*(vel_data - alpha.value * dtrq_data) - (1/r)*trq_data
  vel_m_opt = r*(vel_data - alpha.value * dtrq_data)
  Em_true = integrate.simpson((1/(K_m*K_m))*tau_m_opt**2 + tau_m_opt*vel_m_opt, t)
  Em_per = (Em_true-Em_0)*100/Em_0
  print(f"alpha_s: {alpha.value:.6f}; Em_obj: {prob.value:.6f}; Em_true: {Em_true:.6f} ({Em_per:.1f}%)")
  ...
  for j in range(0, mc_no):
    ...
    tau_m_new = I_m*r*(acc_data_new - alpha.value * ddtrq_data_new) + b_m*r*(vel_data_new - alpha.value * dtrq_data_new) - (1/r)*trq_data_new
    vel_m_new = r*(vel_data_new - alpha.value * dtrq_data_new)
    Em_new[j] = integrate.simpson((1/(K_m*K_m))*tau_m_new**2 + tau_m_new*vel_m_new, t)
